...
public class ToolBox {
	 ...
	 * 
	 */
	public static double[] CompXY(int latD, int latM, int latS, char latDir, int longD, int longM, int longS,
			char longDir) {
	 ...
	 * 
	 */
	public static double[] SegInter(double x11, double y11, double x12, double y12, double x21, double y21, double x22,
			double y22) {
		double dx1 = x12 - x11;
		double dy1 = y12 - y11;
		double dx2 = x22 - x21;
		...
		if (Math.abs(det) < 1.0e-10) {
			...
			if (Math.abs(dy1 * dxx - dx1 * dyy) < 1.0e-10 && Math.abs(dy2 * dxx - dx2 * dyy) < 1.0e-10) {
				...
				if (bounds1[1] < bounds2[0] || bounds1[0] > bounds2[1] || bounds1[3] < bounds2[2]
						|| bounds1[2] > bounds2[3]) {
	 ...
	 * 
	 */
	public static boolean processACollision(ChargerAeroport ch, String dep1, String arr1, String dep2, String arr2,
			LocalTime t1, LocalTime t2, int dur1, int dur2, int marge) {
		...
		double[] xy1 = CompXY(Integer.valueOf(coord1[2]), Integer.valueOf(coord1[3]), Integer.valueOf(coord1[4]),
				coord1[5].charAt(0), Integer.valueOf(coord1[6]), Integer.valueOf(coord1[7]), Integer.valueOf(coord1[8]),
				coord1[9].charAt(0));
		double x11 = xy1[0];
		double y11 = xy1[1];

		double[] xy2 = CompXY(Integer.valueOf(coord2[2]), Integer.valueOf(coord2[3]), Integer.valueOf(coord2[4]),
				coord2[5].charAt(0), Integer.valueOf(coord2[6]), Integer.valueOf(coord2[7]), Integer.valueOf(coord2[8]),
				coord2[9].charAt(0));
		double x12 = xy2[0];
		double y12 = xy2[1];

		double[] xy3 = CompXY(Integer.valueOf(coord3[2]), Integer.valueOf(coord3[3]), Integer.valueOf(coord3[4]),
				coord3[5].charAt(0), Integer.valueOf(coord3[6]), Integer.valueOf(coord3[7]), Integer.valueOf(coord3[8]),
				coord3[9].charAt(0));
		double x21 = xy3[0];
		double y21 = xy3[1];

		double[] xy4 = CompXY(Integer.valueOf(coord4[2]), Integer.valueOf(coord4[3]), Integer.valueOf(coord4[4]),
				coord4[5].charAt(0), Integer.valueOf(coord4[6]), Integer.valueOf(coord4[7]), Integer.valueOf(coord4[8]),
				coord4[9].charAt(0));
		double x22 = xy4[0];
		double y22 = xy4[1];
		...
		// Vérifier si les deux avions décollent du même point avec un écart de moins de
		// la marge de sécurité en minutes
		if (x11 == x21 && y11 == y21 && Math.abs(Duration.between(t1, t2).toMinutes()) < marge) {
			colliding = true;
		}

		// Vérifier si les deux avions atterrissent au même point avec un écart de moins
		// de la marge de sécurité en minutes
		if (x12 == x22 && y12 == y22
				&& Math.abs(Duration.between(t1.plusMinutes(dur1), t2.plusMinutes(dur2)).toMinutes()) < marge) {
			colliding = true;
		}
		...
		if (x11 == x22 && y11 == y22 && x12 == x21 && y12 == y21) {
			// Vérifier si un avion décolle pendant que l'autre vole ou si un avion décolle
			// avant la marge de sécurité en minutes après l'atterrissage de l'autre
			if ((t1.isAfter(t2) && t1.isBefore(t2Arr.plusMinutes(marge)))
					|| (t2.isAfter(t1) && t2.isBefore(t1Arr.plusMinutes(marge)))) {
				colliding = true; // Collision si l'un décolle pendant que l'autre vole ou si l'un décolle avant
									// la marge de sécurité en minutes après l'atterrissage de l'autre
			}
		}
